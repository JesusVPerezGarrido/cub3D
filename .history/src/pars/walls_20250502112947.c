...
/*   Updated: 2025/05/02 11:29:47 by auloth           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	wave_check(char **map, int y, int x, int *open)
{
	if (map[y][x] == '0')
	{
		...
		if (x - 1 < 0 || c_check(map[y][x - 1]) == 1)
			*open = 1;
		if (map[y + 1] == NULL || x > (int)ft_strlen(map[y + 1])
			|| c_check(map[y + 1][x]) == 1)
			*open = 1;
		if (y - 1 < 0 || x > (int)ft_strlen(map[y - 1]) || c_check(map[y
				- 1][x]) == 1)
			*open = 1;
		if (*open == 1)
			return ;
